...
@Component({
  ...
  template: \`
    <div class="q-wrapper">
      <header class="q-header">
        ...
        <div class="header-title">Bilan de mes mains</div>
      </header>

      <div class="q-content">
        
        <div class="question-block">
          <h2 class="step-title">1. Lésions sur les mains / poignets ?</h2>
        ...
        <div class="question-block">
          <h2 class="step-title">2. Symptômes (sélectionnez-en plusieurs)</h2>
          <div class="tags-grid">
        ...
        <div class="question-block">
          <h2 class="step-title">3. Depuis combien de temps ?</h2>
          <div class="timeline-container">
             <button class="timeline-btn" [class.active]="answers[3] === '< 3 mois'" (click)="answers[3] = '< 3 mois'"><div class="tl-circle"></div><span class="tl-text">< 3 mois</span></button>
             <button class="timeline-btn" [class.active]="answers[3] === '3-12 mois'" (click)="answers[3] = '3-12 mois'"><div class="tl-circle"></div><span class="tl-text">3 - 12 mois</span></button>
             <button class="timeline-btn" [class.active]="answers[3] === '+ 1 an'" (click)="answers[3] = '+ 1 an'"><div class="tl-circle"></div><span class="tl-text">+ 1 an</span></button>
          </div>
        </div>

        <div class="question-block">
          <h2 class="step-title">4. Nombre de poussées (12 derniers mois)</h2>
        ...
        <div class="question-block">
          <h2 class="step-title">5. Impact (1 = Aucun, 5 = Très fort)</h2>
  ...
  styles: [\`
    ...
    .q-header { display: flex; align-items: center; padding: 16px 20px; background: white; box-shadow: 0 2px 8px rgba(0,0,0,0.05); z-index: 10; }
    .header-title { flex: 1; text-align: center; font-weight: 800; font-size: 18px; margin-right: 28px; }
    .back-btn { background: none; border: none; color: var(--primary-color, #00af6c); cursor: pointer; padding: 8px; margin-left:-8px; }
    .q-content { flex: 1; padding: 20px; overflow-y: auto; display: flex; flex-direction: column; gap: 32px; padding-bottom: 60px; }
    .question-block { display: flex; flex-direction: column; }
    .step-title { font-size: 18px; font-weight: 800; color: #111; margin: 0 0 16px 0; }
    .cards-row { display: flex; gap: 12px; }
    .choice-card { flex: 1; background: #FAFAFA; border: 2px solid transparent; border-radius: 16px; padding: 16px 8px; display: flex; flex-direction: column; align-items: center; gap: 12px; cursor: pointer; transition: 0.2s;}
    .choice-card.active { border-color: var(--primary-color, #00af6c); background: rgba(0,175,108,0.05); }
    .icon-wrap { width: 48px; height: 48px; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; transition: 0.2s; }
    ...
    .choice-text { font-size: 16px; font-weight: 800; font-family: 'Rethink Sans', sans-serif;}
    .tags-grid { display: flex; flex-wrap: wrap; gap: 8px; }
    .tag-pill { background: #FAFAFA; border: 2px solid #E5E7EB; padding: 10px 14px; border-radius: 20px; font-size: 14px; font-weight: 600; cursor: pointer; transition: 0.2s; font-family: 'Rethink Sans', sans-serif;}
    ...
    .timeline-btn { display: flex; align-items: center; gap: 12px; background: #FAFAFA; border: 2px solid transparent; padding: 16px; border-radius: 16px; cursor: pointer; transition: 0.2s;}
    .timeline-btn.active { border-color: var(--primary-color, #00af6c); background: rgba(0,175,108,0.05); }
    .tl-circle { width: 20px; height: 20px; border-radius: 50%; border: 3px solid #D1D5DB; background: white; transition: 0.2s; }
    .timeline-btn.active .tl-circle { border-color: var(--primary-color, #00af6c); border-width: 6px; }
    .tl-text { font-size: 16px; font-weight: 700; font-family: 'Rethink Sans', sans-serif;}
    .numbers-row { display: flex; gap: 12px; }
    .num-card { flex: 1; aspect-ratio: 1.2; background: #FAFAFA; border: 2px solid transparent; border-radius: 16px; font-size: 24px; font-weight: 800; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: 0.2s; font-family: 'Rethink Sans', sans-serif;}
    .num-card.active { border-color: var(--primary-color, #00af6c); background: var(--primary-color, #00af6c); color: white; }
    .jauge-container { position: relative; padding: 30px 0 10px; }
    .jauge-track { position: absolute; top: 50%; left: 10px; right: 10px; height: 8px; background: #E5E7EB; border-radius: 4px; transform: translateY(-50%); }
    .jauge-fill { height: 100%; background: var(--primary-color, #00af6c); border-radius: 4px; transition: 0.3s; }
    .jauge-points { position: relative; display: flex; justify-content: space-between; align-items: center; }
    .jauge-pt { width: 24px; height: 24px; border-radius: 50%; background: #E5E7EB; border: 3px solid white; cursor: pointer; position: relative; transition: 0.3s; }
    ...
    .pt-lbl { position: absolute; bottom: -28px; left: 50%; transform: translateX(-50%) scale(0.7); font-weight: 800; font-size: 16px; color: var(--primary-color, #00af6c); }
    .jauge-labels { display: flex; justify-content: space-between; font-weight: 700; font-size: 13px; color: #9CA3AF; margin-top: 8px;}
    ...
    .btn-next:disabled { background: #E5E7EB; color: #9CA3AF; }
  \`]
})
...
@Component({
  ...
  template: \`
    <div class="score-container">
      <img src="/assets/images/visuel-main.png" alt="" class="bg-watermark">
      ...
      <header class="s-header">
        ...
        <span class="s-header-title">Résultats</span>
      </header>

      <div class="s-content">
        <div class="score-badge-wrapper" [class]="severityClass + '-glow'">
          <div class="score-icon-circle" [style.background-color]="severityColor">
            <span class="icon-emoji">{{ getSeverityIcon() }}</span>
          </div>
          <h2 class="score-label" [style.color]="severityColor">{{ getSeverityLabel() }}</h2>
        </div>

        <div class="message-card">
          <p class="main-msg">{{ getScoreMessage() }}</p>
  ...
  styles: [\`
    ...
    .s-back-btn { background: none; border: none; color: var(--primary-color, #00af6c); cursor: pointer; padding: 8px; margin-left: -8px;}
    .s-header-title { font-weight: 800; font-size: 18px; color: #000; margin-left: 12px; }
    ...
    .green-glow .score-icon-circle { box-shadow: 0 12px 32px rgba(46, 204, 113, 0.4); }
    .orange-glow .score-icon-circle { box-shadow: 0 12px 32px rgba(243, 156, 18, 0.4); }
    .red-glow .score-icon-circle { box-shadow: 0 12px 32px rgba(231, 76, 60, 0.4); }
    ...
    .pulse-btn::after {
      ...
      width: 100%; height: 100%; background: rgba(0,175,108,0.3);
