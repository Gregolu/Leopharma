...
code = code.replace(
    ...
    '<h1 class="header-title" $1 style="margin:0; font-size:20px; position:absolute; left:50%; transform:translateX(-50%); font-weight:700;">Mes bilans santé</h1>'
);
// In case the style replacement stacked
code = code.replace(/style="margin:0; font-size:20px;" style=".*?"/, 'style="margin:0; font-size:20px; position:absolute; left:50%; transform:translateX(-50%); font-weight:700;"');

code = code.replace(
    /<div class="notification-icon"[\s\S]*?<\/svg>\s*<\/div>/,
...
code = code.replace(
    /<div class="profile-icon"[\s\S]*?<\/svg>\s*<\/div>/,
...
dcode = dcode.replace(
    ...
    '<button class="primary-btn center-btn mt-3" style="background:#111; color:white; border:none; width:100%; border-radius:30px; font-weight:700; padding:16px;" (click)="goTo(\\'/questionnaire\\')">Mettre à jour mon dossier</button>'
);

// Voir mon dossier (green button)
dcode = dcode.replace(
    /<button class="primary-btn center-btn mt-4" \(click\)="goTo\('\/dossier'\)">Voir mon dossier<\/button>/,
    '<button class="primary-btn center-btn mt-4" style="background:#00854d; color:white; border:none; width:100%; border-radius:30px; font-weight:700; padding:16px;" (click)="goTo(\\'/dossier\\')">Voir mon dossier</button>'
);
